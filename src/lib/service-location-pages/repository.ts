...
import { getAppMode, getTemplateProfileId } from '@/lib/template/profile'
import { validateLocationPage } from '@/lib/template/validate'

function getDataDir() {
  const profileId = getTemplateProfileId()
  return path.join(process.cwd(), 'profiles', profileId, 'locations')
}
...
async function loadAllPages(): Promise<ServiceLocationPageData[]> {
  ...
  const pages = await Promise.all(
    files.map(async (file) => {
      ...
      const errors = validateLocationPage(parsed, getAppMode())
      if (errors.length > 0) {
        throw new Error(`Invalid location page data in ${file}:\n- ${errors.join('\n- ')}`)
      }
